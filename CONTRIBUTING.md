...
The rfc-xmldiff project uses the **Git Feature Workflow** model.
...
As a general rule, work is never done directly on the rfc-xmldiff repository. You instead [create a fork](https://docs.github.com/en/get-started/quickstart/fork-a-repo) of the project. Creating a "fork" is producing a personal copy of the rfc-xmldiff project. Forks act as a sort of bridge between the original repository and your personal copy.

1. Navigate to https://github.com/ietf-tools/rfc-xmldiff
...
Right now, you have a fork of the rfc-xmldiff repository, but you don't have the files in that repository locally on your computer.

After forking the rfc-xmldiff repository, you should have landed on your personal forked copy. If that's not the case, make sure you are on the fork (e.g. `john-doe/rfc-xmldiff` and not the original repository `ietf-tools/rfc-xmldiff`).
...
```sh
git clone https://github.com/YOUR-USERNAME/rfc-xmldiff
```
4. Press **Enter**. Your local clone will be created in a subfolder named `rfc-xmldiff`.
...
2. In a terminal window, navigate to where you want to work. Subfolders will be created for each project you clone. **DO NOT** create empty folders for projects to be cloned. This is done automatically by git.
3. Type `gh repo clone` followed by `YOUR-USERNAME/rfc-xmldiff` (replacing YOUR-USERNAME with your GitHub username), e.g.:
```sh
gh repo clone john-doe/rfc-xmldiff
```
4. Press **Enter**. Your local clone will be created in a subfolder named `rfc-xmldiff`.
...
Make sure the base repository is set to `ietf-tools/rfc-xmldiff` with the branch `main` (this is the destination):
